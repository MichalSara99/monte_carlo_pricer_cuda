...
namespace gbm_kernels {
	...
	namespace euler_scheme {
		...
		__global__
			void generatePathsKernelDouble1D(sde_builder_cuda::GeometricBrownianMotion<double> gbm,
		...
		__global__
			void generatePathsKernelFloat1D(sde_builder_cuda::GeometricBrownianMotion<float> gbm,
		...
		__global__
			void generatePathsKernelDouble2D(sde_builder_cuda::GeometricBrownianMotion<double> gbm,
				double *d_paths, curandState_t* states,
		...
		__global__
			void generatePathsKernelFloat2D(sde_builder_cuda::GeometricBrownianMotion<float> gbm,
				float *d_paths, curandState_t* states,
		...
		__global__
			void generatePathsKernelDouble3D(sde_builder_cuda::GeometricBrownianMotion<double> gbm,
				double *d_paths, curandState_t* states,
		...
		__global__
			void generatePathsKernelFloat3D(sde_builder_cuda::GeometricBrownianMotion<float> gbm,
				float *d_paths, curandState_t* states,
				unsigned int nPathsWidth, unsigned int nPathsHeight, unsigned int nPathsDepth,
...
namespace fdm_engine_cuda {
	...
	void fdm_engine_cuda::GBMPathEngineDouble::_generate1D(double *d_paths, curandState_t *states, FDMScheme scheme,
		...
		{
			gbm_kernels::euler_scheme::generatePathsKernelDouble1D << <threadsPerBlock, blocksPerGrid >> > (this->gbm_, d_paths, states,
				nPaths, nSteps, dt);
		...
		{
			gbm_kernels::milstein_scheme::generatePathsKernelDouble1D << <threadsPerBlock, blocksPerGrid >> > (this->gbm_, d_paths, states,
				nPaths, nSteps, dt);
		}
		break;
		}
	...
	void fdm_engine_cuda::GBMPathEngineDouble::_generate2D(double *d_paths, curandState_t *states, FDMScheme scheme,
		...
		case FDMScheme::EulerScheme:
		{
			gbm_kernels::euler_scheme::generatePathsKernelDouble2D << <gridSize, blockSize >> > (this->gbm_, d_paths, states,
				widthSize, heightSize, nSteps, dt);
		}
		break;
		case FDMScheme::MilsteinScheme:
		{
			gbm_kernels::milstein_scheme::generatePathsKernelDouble2D << <gridSize, blockSize >> > (this->gbm_, d_paths, states,
				widthSize, heightSize, nSteps, dt);
		}
		break;
		}
	...
	void fdm_engine_cuda::GBMPathEngineDouble::_generate3D(double *d_paths, curandState_t *states, FDMScheme scheme,
		...
		case FDMScheme::EulerScheme:
		{
			gbm_kernels::euler_scheme::generatePathsKernelDouble3D << <gridSize, blockSize >> > (this->gbm_, d_paths, states,
				widthSize, heightSize, depthSize, nSteps, dt);
		}
		break;
		case FDMScheme::MilsteinScheme:
		{
			gbm_kernels::milstein_scheme::generatePathsKernelDouble3D << <gridSize, blockSize >> > (this->gbm_, d_paths, states,
				widthSize, heightSize, depthSize, nSteps, dt);
		}
		break;
		}
	...
	void fdm_engine_cuda::GBMPathEngineFloat::_generate1D(float *d_paths, curandState_t *states, FDMScheme scheme,
		...
		case FDMScheme::EulerScheme:
		{
			gbm_kernels::euler_scheme::generatePathsKernelFloat1D << <threadsPerBlock, blocksPerGrid >> > (this->gbm_, d_paths, states,
				nPaths, nSteps, dt);
		}
		break;
		case FDMScheme::MilsteinScheme:
		{
			gbm_kernels::milstein_scheme::generatePathsKernelFloat1D << <threadsPerBlock, blocksPerGrid >> > (this->gbm_, d_paths, states,
				nPaths, nSteps, dt);
		}
		break;
		}
	...
	void fdm_engine_cuda::GBMPathEngineFloat::_generate2D(float *d_paths, curandState_t *states, FDMScheme scheme,
		...
		case FDMScheme::EulerScheme:
		{
			gbm_kernels::euler_scheme::generatePathsKernelFloat2D << <gridSize, blockSize >> > (this->gbm_, d_paths, states,
				widthSize, heightSize, nSteps, dt);
		}
		break;
		case FDMScheme::MilsteinScheme:
		{
			gbm_kernels::milstein_scheme::generatePathsKernelFloat2D << <gridSize, blockSize >> > (this->gbm_, d_paths, states,
				widthSize, heightSize, nSteps, dt);
		}
		break;
		}
	...
	void fdm_engine_cuda::GBMPathEngineFloat::_generate3D(float *d_paths, curandState_t *states, FDMScheme scheme,
		...
		case FDMScheme::EulerScheme:
		{
			gbm_kernels::euler_scheme::generatePathsKernelFloat3D << <gridSize, blockSize >> > (this->gbm_, d_paths, states,
				widthSize, heightSize, depthSize, nSteps, dt);
		}
		break;
		case FDMScheme::MilsteinScheme:
		{
			gbm_kernels::milstein_scheme::generatePathsKernelFloat3D << <gridSize, blockSize >> > (this->gbm_, d_paths, states,
				widthSize, heightSize, depthSize, nSteps, dt);
		}
		break;
		}
