...
namespace abm_kernels {
	...
	namespace euler_scheme {
		...
		__global__
			void generatePathsKernelDouble1D(sde_builder_cuda::ArithmeticBrownianMotion<double> abm,
		...
		__global__
			void generatePathsKernelFloat1D(sde_builder_cuda::ArithmeticBrownianMotion<float> abm,
		...
		__global__
			void generatePathsKernelDouble2D(sde_builder_cuda::ArithmeticBrownianMotion<double> abm,
				double *d_paths, curandState_t* states,
		...
		__global__
			void generatePathsKernelFloat2D(sde_builder_cuda::ArithmeticBrownianMotion<float> abm,
				float *d_paths, curandState_t* states,
		...
		__global__
			void generatePathsKernelDouble3D(sde_builder_cuda::ArithmeticBrownianMotion<double> abm,
				double *d_paths, curandState_t* states,
		...
		__global__
			void generatePathsKernelFloat3D(sde_builder_cuda::ArithmeticBrownianMotion<float> abm,
				float *d_paths, curandState_t* states,
				unsigned int nPathsWidth, unsigned int nPathsHeight, unsigned int nPathsDepth,
...
namespace fdm_engine_cuda {
	...
	void fdm_engine_cuda::ABMPathEngineDouble::_generate1D(double *d_paths, curandState_t *states, FDMScheme scheme,
		...
		{
			abm_kernels::euler_scheme::generatePathsKernelDouble1D << <threadsPerBlock, blocksPerGrid >> > (this->abm_, d_paths, states,
				nPaths, nSteps, dt);
		...
		{
			abm_kernels::milstein_scheme::generatePathsKernelDouble1D << <threadsPerBlock, blocksPerGrid >> > (this->abm_, d_paths, states,
				nPaths, nSteps, dt);
		}
		break;
		}
	...
	void fdm_engine_cuda::ABMPathEngineDouble::_generate2D(double *d_paths, curandState_t *states, FDMScheme scheme,
		unsigned int nPaths, unsigned int nSteps, double dt)const {
		...
		case FDMScheme::EulerScheme:
		{
			abm_kernels::euler_scheme::generatePathsKernelDouble2D << <gridSize, blockSize >> > (this->abm_, d_paths, states,
				widthSize, heightSize, nSteps, dt);
		}
		break;
		case FDMScheme::MilsteinScheme:
		{
			abm_kernels::milstein_scheme::generatePathsKernelDouble2D << <gridSize, blockSize >> > (this->abm_, d_paths, states,
				widthSize, heightSize, nSteps, dt);
		}
		break;
		}
	...
	void fdm_engine_cuda::ABMPathEngineDouble::_generate3D(double *d_paths, curandState_t *states, FDMScheme scheme,
		unsigned int nPaths, unsigned int nSteps, double dt)const {
		...
		case FDMScheme::EulerScheme:
		{
			abm_kernels::euler_scheme::generatePathsKernelDouble3D << <gridSize, blockSize >> > (this->abm_, d_paths, states,
				widthSize, heightSize, depthSize, nSteps, dt);
		}
		break;
		case FDMScheme::MilsteinScheme:
		{
			abm_kernels::milstein_scheme::generatePathsKernelDouble3D << <gridSize, blockSize >> > (this->abm_, d_paths, states,
				widthSize, heightSize, depthSize, nSteps, dt);
		}
		break;
		}
	...
	void fdm_engine_cuda::ABMPathEngineFloat::_generate1D(float *d_paths, curandState_t *states, FDMScheme scheme,
		...
		case FDMScheme::EulerScheme:
		{
			abm_kernels::euler_scheme::generatePathsKernelFloat1D << <threadsPerBlock, blocksPerGrid >> > (this->abm_, d_paths, states,
				nPaths, nSteps, dt);
		}
		break;
		case FDMScheme::MilsteinScheme:
		{
			abm_kernels::milstein_scheme::generatePathsKernelFloat1D << <threadsPerBlock, blocksPerGrid >> > (this->abm_, d_paths, states,
				nPaths, nSteps, dt);
		}
		break;
		}
	...
	void fdm_engine_cuda::ABMPathEngineFloat::_generate2D(float *d_paths, curandState_t *states, FDMScheme scheme,
		unsigned int nPaths, unsigned int nSteps, float dt)const {
		const unsigned int widthSize{ 1000 };
		assert((nPaths%widthSize) == 0);
		unsigned int heightSize{ nPaths / widthSize };
		...
		case FDMScheme::EulerScheme:
		{
			abm_kernels::euler_scheme::generatePathsKernelFloat2D << <gridSize, blockSize >> > (this->abm_, d_paths, states,
				widthSize, heightSize, nSteps, dt);
		}
		break;
		case FDMScheme::MilsteinScheme:
		{
			abm_kernels::milstein_scheme::generatePathsKernelFloat2D << <gridSize, blockSize >> > (this->abm_, d_paths, states,
				widthSize, heightSize, nSteps, dt);
		}
		break;
		}
	...
	void fdm_engine_cuda::ABMPathEngineFloat::_generate3D(float *d_paths, curandState_t *states, FDMScheme scheme,
		unsigned int nPaths, unsigned int nSteps, float dt)const {
		const unsigned int widthSize{ 100 };
		const unsigned int heightSize{ 100 };
		assert((nPaths % (widthSize*heightSize)) == 0);
		...
		case FDMScheme::EulerScheme:
		{
			abm_kernels::euler_scheme::generatePathsKernelFloat3D << <gridSize, blockSize >> > (this->abm_, d_paths, states,
				widthSize, heightSize, depthSize, nSteps, dt);
		}
		break;
		case FDMScheme::MilsteinScheme:
		{
			abm_kernels::milstein_scheme::generatePathsKernelFloat3D << <gridSize, blockSize >> > (this->abm_, d_paths, states,
				widthSize, heightSize, depthSize, nSteps, dt);
		}
		break;
		}
